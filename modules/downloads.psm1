function Get-Binary
{
    ...
    if(-Not(Get-Command $command -ErrorAction SilentlyContinue))
    {
        ...
        if([string]::IsNullOrEmpty($override))
        {
            if($preRelease)
            {
                $sourceURI = Get-GitLatestReleaseURI $sourceRepo -n $namePattern -preRelease
            }
            else
            {
                $sourceURI = Get-GitLatestReleaseURI $sourceRepo -n $namePattern
            }
        }
        else
        {
        ...
        {
            $destFolder = Join-Path $libFolder -ChildPath $zipFolderName
        }
        else
        {
            $destFolder = (Join-Path $libFolder -ChildPath $sourceRepo)
        }
